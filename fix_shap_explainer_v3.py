#!/usr/bin/env python3
"""
Fix SHAP explainer with proper approach
"""
...
def regenerate_shap_explainer_v3():
    """Regenerate SHAP explainer with proper approach"""
    
    # Change to M directory where model files are located
    os.chdir('M')
    
    print("🔧 Regenerating SHAP explainer (v3)...")
    ...
    try:
        ...
        n_samples = 50  # Smaller for efficiency
    ...
    try:
        ...
        explainer = shap.TreeExplainer(model, X_background_scaled)
        print("✅ SHAP TreeExplainer created successfully")
        
        # Test the explainer with a sample (disable additivity check in shap_values call)
        print("🧪 Testing SHAP explainer...")
        test_sample = X_background_scaled[:1]  # Use first sample as test
        
        # Try to get SHAP values with error handling
        try:
            shap_values = explainer.shap_values(test_sample, check_additivity=False)
        except TypeError:
            # If check_additivity parameter doesn't exist, try without it
...
if __name__ == "__main__":
    print("🔧 SHAP Explainer Fix Tool v3")
    print("=" * 40)
    
    success = regenerate_shap_explainer_v3()
    ...
    if success:
        ...
        print("2. Restart: python M/Production_Sepsis_API.py")
        print("3. Test SHAP: python test_shap_functionality.py")
    else:
